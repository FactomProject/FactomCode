...
// Factom adaptation: the main chain is the Directory Block chain
...
func TestGetBlocks(t *testing.T) {
	...
	locatorHash, err := factomwire.NewShaHashFromStr(hashStr)
	if err != nil {
	...
	hashStop, err := factomwire.NewShaHashFromStr(hashStr)
	if err != nil {
		t.Errorf("NewShaHashFromStr: %v", err)
	}

	// Ensure we get the same data back out.
	msg := factomwire.NewMsgGetBlocks(hashStop)
	...
	for i := 0; i < factomwire.MaxBlockLocatorsPerMsg; i++ {
		err = msg.AddBlockLocatorHash(locatorHash)
	}
	if err == nil {
...
func TestGetBlocksWire(t *testing.T) {
	...
	hashLocator, err := factomwire.NewShaHashFromStr(hashStr)
	if err != nil {
		t.Errorf("NewShaHashFromStr: %v", err)
	}
	...
	hashLocator2, err := factomwire.NewShaHashFromStr(hashStr)
	if err != nil {
		t.Errorf("NewShaHashFromStr: %v", err)
	}
	...
	noLocators := factomwire.NewMsgGetBlocks(&factomwire.ShaHash{})
	...
	multiLocators := factomwire.NewMsgGetBlocks(hashStop)
	...
	tests := []struct {
		in   *factomwire.MsgGetBlocks // Message to encode
		out  *factomwire.MsgGetBlocks // Expected decoded message
	...
	for i, test := range tests {
		...
		var msg factomwire.MsgGetBlocks
...
func TestGetBlocksWireErrors(t *testing.T) {
	...
	// MsgGetBlocks message with multiple block locators and a stop hash.
	baseGetBlocks := factomwire.NewMsgGetBlocks(hashStop)
	...
	maxGetBlocks := factomwire.NewMsgGetBlocks(hashStop)
	for i := 0; i < factomwire.MaxBlockLocatorsPerMsg; i++ {
	...
	tests := []struct {
		in       *factomwire.MsgGetBlocks // Value to encode
		buf      []byte                   // Wire encoding
		pver     uint32                   // Protocol version for wire encoding
		...
		{maxGetBlocks, maxGetBlocksEncoded, pver, 7, factomwireErr, factomwireErr},
	}
