...
var mainNetGenesisHash = factomwire.ShaHash([factomwire.HashSize]byte{ // Make go vet happy.
...
var mainNetGenesisMerkleRoot = factomwire.ShaHash([factomwire.HashSize]byte{ // Make go vet happy.
...
func TestElementWire(t *testing.T) {
	...
	tests := []struct {
		...
		{
			(*factomwire.ShaHash)(&[factomwire.HashSize]byte{ // Make go vet happy.
				0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
				0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
		...
		{
			factomwire.ServiceFlag(factomwire.SFNodeNetwork),
			[]byte{0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
		},
		{
			factomwire.InvType(factomwire.InvTypeTx),
			[]byte{0x01, 0x00, 0x00, 0x00},
		},
		/*
			{
				factomwire.BitcoinNet(factomwire.MainNet),
	...
	for i, test := range tests {
		...
		err := factomwire.TstWriteElement(&buf, test.in)
		if err != nil {
		...
		err = factomwire.TstReadElement(rbuf, val)
		if err != nil {
...
func TestElementWireErrors(t *testing.T) {
	tests := []struct {
		...
		{factomwire.ServiceFlag(factomwire.SFNodeNetwork), 0, io.ErrShortWrite, io.EOF},
		{factomwire.InvType(factomwire.InvTypeTx), 0, io.ErrShortWrite, io.EOF},
		//		{factomwire.BitcoinNet(factomwire.MainNet), 0, io.ErrShortWrite, io.EOF},
	}
	...
	for i, test := range tests {
		...
		err := factomwire.TstWriteElement(w, test.in)
		...
		err = factomwire.TstReadElement(r, val)
		if err != test.readErr {
...
func TestVarIntWire(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := factomwire.TstWriteVarInt(&buf, test.pver, test.in)
		if err != nil {
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		val, err := factomwire.TstReadVarInt(rbuf, test.pver)
		if err != nil {
...
func TestVarIntWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		w := newFixedWriter(test.max)
		err := factomwire.TstWriteVarInt(w, test.pver, test.in)
		if err != test.writeErr {
		...
		_, err = factomwire.TstReadVarInt(r, test.pver)
		if err != test.readErr {
...
func TestVarIntSerializeSize(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		serializedSize := factomwire.VarIntSerializeSize(test.val)
...
func TestVarStringWire(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := factomwire.TstWriteVarString(&buf, test.pver, test.in)
		if err != nil {
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		val, err := factomwire.TstReadVarString(rbuf, test.pver)
		if err != nil {
...
func TestVarStringWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		w := newFixedWriter(test.max)
		err := factomwire.TstWriteVarString(w, test.pver, test.in)
		if err != test.writeErr {
		...
		// Decode from wire format.
		r := newFixedReader(test.max, test.buf)
		_, err = factomwire.TstReadVarString(r, test.pver)
		if err != test.readErr {
...
func TestVarStringOverflowErrors(t *testing.T) {
	...
	for i, test := range tests {
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		_, err := factomwire.TstReadVarString(rbuf, test.pver)
...
func TestVarBytesWire(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := factomwire.TstWriteVarBytes(&buf, test.pver, test.in)
		if err != nil {
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		val, err := factomwire.TstReadVarBytes(rbuf, test.pver,
			factomwire.MaxMessagePayload, "test payload")
		if err != nil {
...
func TestVarBytesWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		w := newFixedWriter(test.max)
		err := factomwire.TstWriteVarBytes(w, test.pver, test.in)
		if err != test.writeErr {
		...
		// Decode from wire format.
		r := newFixedReader(test.max, test.buf)
		_, err = factomwire.TstReadVarBytes(r, test.pver,
			factomwire.MaxMessagePayload, "test payload")
		if err != test.readErr {
...
func TestVarBytesOverflowErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		_, err := factomwire.TstReadVarBytes(rbuf, test.pver,
			factomwire.MaxMessagePayload, "test payload")
		t.Logf("here 222")
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
...
func TestRandomUint64Errors(t *testing.T) {
	...
	nonce, err := factomwire.TstRandomUint64(fr)
