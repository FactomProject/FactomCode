...
func TestECAddress(t *testing.T) {
	...
	if len(pk) != len(decoded) { t.Fatalf("len(pk) != len(decoded)") }

    for i := range pk {
        if pk[i] != decoded[i] { t.Fatalf("pk != decoded") }
    }

    if ver != netid { t.Fatalf("ver != netid")}
}
...
func TestFactoidAddress(t *testing.T) {
	...
	if len(hash.Bytes) != len(decoded) { t.Fatalf("len(hash.Bytes) != len(decoded)") }

    for i := range hash.Bytes {
        if hash.Bytes[i] != decoded[i] { t.Fatalf("hash.Bytes != decoded") }
    }
