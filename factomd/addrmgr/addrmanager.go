...
func (a *AddrManager) getNewBucket(netAddr, srcAddr *factomwire.NetAddress) int {
	...
	hash1 := factomwire.DoubleSha256(data1)
	...
	hash2 := factomwire.DoubleSha256(data2)
...
out:
	...
	a.savePeers()
	a.wg.Done()
	fmt.Println("Address handler done")
}
...
func (a *AddrManager) DeserializeNetAddress(addr string) (*factomwire.NetAddress, error) {
	...
	return a.HostToNetAddress(host, uint16(port), factomwire.SFNodeNetwork)
}
...
func (a *AddrManager) Start() {
	...
	fmt.Println("Starting address manager")
...
func (a *AddrManager) AddressCache() []*factomwire.NetAddress {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	...
	allAddr := make([]*factomwire.NetAddress, a.nNew+a.nTried)
	i := 0
...
func (a *AddrManager) HostToNetAddress(host string, port uint16, services factomwire.ServiceFlag) (*factomwire.NetAddress, error) {
...
func NetAddressKey(na *factomwire.NetAddress) string {
...
func (a *AddrManager) find(addr *factomwire.NetAddress) *knownAddress {
	return a.addrIndex[NetAddressKey(addr)]
}

// Attempt increases the given address' attempt counter and updates
// the last attempt time.
func (a *AddrManager) Attempt(addr *factomwire.NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
// be ignored.
func (a *AddrManager) Connected(addr *factomwire.NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *AddrManager) Good(addr *factomwire.NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
...
func (a *AddrManager) AddLocalAddress(na *factomwire.NetAddress, priority AddressPriority) error {
	if !IsRoutable(na) {
...
func getReachabilityFrom(localAddr, remoteAddr *factomwire.NetAddress) int {
	const (
		Unreachable = 0
		Default     = iota
		Teredo
...
func (a *AddrManager) GetBestLocalAddress(remoteAddr *factomwire.NetAddress) *factomwire.NetAddress {
	a.lamtx.Lock()
	defer a.lamtx.Unlock()

	bestreach := 0
	var bestscore AddressPriority
	var bestAddress *factomwire.NetAddress
	...
	if bestAddress != nil {
		...
		bestAddress = &factomwire.NetAddress{
			Timestamp: time.Now(),
