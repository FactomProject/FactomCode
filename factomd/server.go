...
const (
	...
	// server.
	supportedServices = factomwire.SFNodeNetwork
...
type broadcastMsg struct {
	message      factomwire.Message
	excludePeers []*peer
}

// broadcastInventoryAdd is a type used to declare that the InvVect it contains
// needs to be added to the rebroadcast map
type broadcastInventoryAdd *factomwire.InvVect

// broadcastInventoryDel is a type used to declare that the InvVect it contains
// needs to be removed from the rebroadcast map
type broadcastInventoryDel *factomwire.InvVect
...
func (s *server) AddRebroadcastInventory(iv *factomwire.InvVect) {
...
func (s *server) RemoveRebroadcastInventory(iv *factomwire.InvVect) {
	// Ignore if shutting down.
	if atomic.LoadInt32(&s.shutdown) != 0 {
		return
	}
...
func (s *server) seedFromDNS() {
	...
	for _, seeder := range activeNetParams.dnsSeeds {
		go func(seeder string) {
			...
			addresses := make([]*factomwire.NetAddress, len(seedpeers))
			// if this errors then we have *real* problems
			intPort, _ := strconv.Atoi(activeNetParams.DefaultPort)
			for i, peer := range seedpeers {
				addresses[i] = new(factomwire.NetAddress)
...
func (s *server) rebroadcastHandler() {
	...
	pendingInvs := make(map[factomwire.InvVect]struct{})
...
func newServer(listenAddrs []string, netParams *factomnet.Params) (*server, error) {
