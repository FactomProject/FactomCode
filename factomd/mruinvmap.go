...
func (m *MruInventoryMap) Exists(iv *factomwire.InvVect) bool {
...
func (m *MruInventoryMap) Add(iv *factomwire.InvVect) {
	...
	if uint(len(m.invMap))+1 > m.limit {
		node := m.invList.Back()
		lru, ok := node.Value.(*factomwire.InvVect)
		if !ok {
			return
		}
...
func (m *MruInventoryMap) Delete(iv *factomwire.InvVect) {
	if node, exists := m.invMap[*iv]; exists {
...
func NewMruInventoryMap(limit uint) *MruInventoryMap {
	m := MruInventoryMap{
		invMap:  make(map[factomwire.InvVect]*list.Element),
		invList: list.New(),
		limit:   limit,
	}
	return &m
