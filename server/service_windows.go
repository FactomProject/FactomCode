...
const (
	// svcName is the name of factomd service.
	svcName = "factomdsvc"
	...
	svcDisplayName = "Factomd Service"
...
// logServiceStartOfDay logs information about factomd when the main server has
...
// factomdService houses the main service handler which handles all service
// updates and launching factomdMain.
type factomdService struct{}
...
// long-running factomdMain (which is the real meat of factomd), handles service
// change requests, and notifies the service control manager of changes.
func (s *factomdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start factomdMain in a separate goroutine so the service can start
	...
	for {
		...
		case c := <-r:
			...
			case svc.Stop, svc.Shutdown:
				...
				// factomdMain to exit in the goroutine above which
...
// installService attempts to install the factomd service.  Typically this should
...
// removeService attempts to uninstall the factomd service.  Typically this should
...
// startService attempts to start the factomd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &factomdService{})
	if err != nil {
