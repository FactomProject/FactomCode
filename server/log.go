...
const (
	...
	lockTimeThreshold int64 = 5e8 // Tue Nov 5 00:53:20 1985 UTC
...
func invSummary(invList []*wire.InvVect) string {
	...
	if invLen == 1 {
		...
		case wire.InvTypeFactomEntryCreditBlock:
			return fmt.Sprintf("factom EntryCreditBlock %s", iv.Hash)
		case wire.InvTypeFactomAdminBlock:
			return fmt.Sprintf("factom AdminBlock %s", iv.Hash)
...
func messageSummary(msg wire.Message) string {
	...
	case *wire.MsgVersion:
		return fmt.Sprintf("id %s, state %d, type %s, block %d, start %d",
			msg.NodeID, msg.NodeState, msg.NodeType, msg.LastBlock, msg.StartTime)
