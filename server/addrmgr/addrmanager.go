...
func (a *AddrManager) expireNew(bucket int) {
	...
	// factomd here chooses four random and just throws the oldest of
...
// We just choose the eldest. factomd selects 4 random entries and throws away
// the older of them.
...
func (a *AddrManager) addressHandler() {
	a.wg.Add(1)
	defer func() {
		//fmt.Println("wg.Done for addressHandler")
		a.wg.Done()
	}()
...
func (a *AddrManager) HostToNetAddress(host string, port uint16, services wire.ServiceFlag) (*wire.NetAddress, error) {
	...
	if len(host) == 22 && host[16:] == ".onion" {
		...
		// but tor and factomd tend to user lowercase, so we switch
		// case here.
...
// New returns a new factom address manager.
