...
func (k *FileKeyStore) Save(session string, keyRing *KeyRing) error {
	...
	keyRing.Each(func(keyPair *KeyPair) {
		privateKeys = append(privateKeys, util.Bytes2Hex(keyPair.PrivateKey))
		publicKeys = append(publicKeys, util.Bytes2Hex(keyPair.PublicKey))
		addresses = append(addresses, util.Bytes2Hex(keyPair.Address()))
