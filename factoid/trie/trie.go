...
func ParanoiaCheck(t1 *Trie) (bool, *Trie) {
	...
	t1.NewIterator().Each(func(key string, v *util.Value) {
		t2.Update(key, v.Str())
	})

	a := util.NewValue(t2.Root).Bytes()
	b := util.NewValue(t1.Root).Bytes()
...
func NewNode(key []byte, val *util.Value, dirty bool) *Node {
...
func (cache *Cache) Get(key []byte) *util.Value {
	...
	value := util.NewValueFromBytes(data)
...
// A (modified) Radix Trie implementation. The Trie implementsS
...
func New(db util.Database, Root interface{}) *Trie {
...
func (t *Trie) Get(key string) string {
	...
	k := CompactHexDecode(key)
	c := util.NewValue(t.getState(t.Root, k))
...
func (t *Trie) Cmp(trie *Trie) bool {
	return util.NewValue(t.Root).Cmp(util.NewValue(trie.Root))
}
...
func (t *Trie) getNode(node interface{}) *util.Value {
	n := util.NewValue(node)
	...
	if len(str) == 0 {
		...
		return util.NewValueFromBytes([]byte(str))
	}
...
func (it *TrieIterator) workNode(currentNode *util.Value) {
	if currentNode.Len() == 2 {
		k := CompactDecode(currentNode.Get(0).Str())
...
func (it *TrieIterator) Collect() [][]byte {
	...
	it.getNode(util.NewValue(it.trie.Root).Bytes())
...
type EachCallback func(key string, node *util.Value)

func (it *TrieIterator) Each(cb EachCallback) {
	it.fetchNode(nil, util.NewValue(it.trie.Root).Bytes(), cb)
}
...
func (it *TrieIterator) iterateNode(key []int, currentNode *util.Value, cb EachCallback) {
	if currentNode.Len() == 2 {
		k := CompactDecode(currentNode.Get(0).Str())
